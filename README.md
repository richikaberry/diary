...
### リサーチツール
#### リサーチツールの画面作成を担当
 - storybookを使用して、コンポーネントを画面作為に先んじて作成しデザインの方向性の統一をしました
 - pnpmを今回使用しました
 - tailwindを使用してデザインを行いました
...
- HTML要素の適切化
  - パラグラフタグをどこで使用するか
    - 他要素もそれぞれの用途を確立
  - BEM名称の適切化
    - 端的に何をしているかを名称から推測出来るようにする
- コンポーネント化
  - コンポーネントをネストして複数個表示されているグラフ表示を抽象化した対応をしたり、逆にネストを無くし読者に簡潔に分かるようにしたり
- 関数の範囲の特定
  - prismaから呼び出す際にaggregateやcountを使用してある程度データを集計した状態で取得し、それを応用すること
  - 全取得する際の要件を決めておくこと
